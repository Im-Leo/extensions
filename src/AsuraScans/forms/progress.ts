...
import { AS_API_DOMAIN } from "../models";
import {
  readingStatuses,
  type AsuraBookmark,
...
/** Reads and writes the reading progress AsuraScans stores against a bookmark. */
      ...
      .then((token) => {
        ...
        Application.scheduleRequest(request)
          .then(async ([res, buffer]) => {
            ...
            this.new = this.asuraTitle === undefined || this.asuraTitle.last_read_chapter == 0;
            ...
            if (this.asuraTitle === undefined) {
              ...
              // Awaited: the reloadForm() below is what renders `this.error`, so an
              // unawaited failure would render as a success the user never got.
              const [created] = await Application.scheduleRequest(request);
