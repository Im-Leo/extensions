...
import type { CustomListResponse } from "./models";
import { fetchJSON } from "./network";
import { filterMangaRelationships } from "./relationships";
import { buildCustomListUrl, buildUserListsUrl } from "./urls";

/** The admin account owns every curated list the homepage shows. */
...
function parseSeasonalName(name: string): ParsedSeason | null {
  const [, season, year] = name.match(SEASONAL_RE) ?? [];
  if (season === undefined || year === undefined) return null;

  return { season: season.toLowerCase() as SeasonName, year: parseInt(year, 10) };
}

/**
 * The season underway, not the newest list on the site: a "Summer" list appears
 * in late spring and must not displace the season actually running.
 */
...
function discoverFromResponse(response: UserListsResponse): Partial<CuratedListIds> {
  ...
  for (const entry of response.data ?? []) {
    ...
    if (!name) continue;
    const count = filterMangaRelationships(entry.relationships).length;
    if (count === 0) continue;
...
export async function fetchCustomListMangaIds(listId: string): Promise<string[] | null> {
  const json = await fetchJSON<CustomListResponse>({
    url: buildCustomListUrl(listId),
    method: "GET",
  });
  if (!json.data || !Array.isArray(json.data.relationships)) return null;
  return filterMangaRelationships(json.data.relationships).map((r) => r.id);
}
