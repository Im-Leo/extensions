...
const STATUS_OPTIONS = STATUSES.map((s) => ({ id: s.enum, title: s.name }));

const skipCountOptions = (noun: string): { id: string; title: string }[] => [
...
const SKIP_COUNT_OPTIONS_NEW = skipCountOptions("new");
const SKIP_COUNT_OPTIONS_UNREAD = skipCountOptions("unread");

const parseSelectInt = (value: string[], fallback: number): number => {
  const selected = value[0];
  const parsed = selected !== undefined ? parseInt(selected, 10) : NaN;
...
/** How library updates behave: batching, and which titles are skipped. */
      ...
      Section(
        ...
        [
          ...
          SelectRow("skip_new_chapters", {
            ...
            options: SKIP_COUNT_OPTIONS_NEW,
            minItemCount: 1,
            maxItemCount: 1,
            onValueChange: Application.Selector(
              this as UpdateFilterSettingsForm,
          ...
          SelectRow("skip_unread_chapters", {
            ...
            options: SKIP_COUNT_OPTIONS_UNREAD,
            minItemCount: 1,
            maxItemCount: 1,
            onValueChange: Application.Selector(
              this as UpdateFilterSettingsForm,
            ...
            subtitle:
              skipPublicationStatus.length > 0
                ? `Skipping: ${skipPublicationStatus
                    .map((status) => STATUSES.find((s) => s.enum === status)?.name ?? status)
                    .join(", ")}`
