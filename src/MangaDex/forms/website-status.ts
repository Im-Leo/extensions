...
/** Module-level, so navigating back and forth does not refetch. Refresh bypasses it. */
...
export class WebsiteStatusForm extends Form {
  ...
  // Only the newest fetch may write, so a slow load cannot overwrite a refresh.
    ...
    try {
      ...
      const parseUtc = (datePart: string, timePart: string): Date => {
        ...
        if (monthName !== undefined && day !== undefined && year !== undefined) {
          ...
          if (month !== undefined && hour !== undefined && minute !== undefined) {
            return new Date(
              Date.UTC(
                Number(year),
                month,
      ...
      const convertToLocalTime = (utcTimestamp: string): string => {
        ...
        try {
          const parts = utcTimestamp.replace(" UTC", "").split(" - ");
          const [datePart, timePart] = parts;
          if (parts.length !== 2 || datePart === undefined || timePart === undefined) {
            return utcTimestamp;
          }

          const date = parseUtc(datePart, timePart);
