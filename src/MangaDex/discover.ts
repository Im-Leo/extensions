...
import { ensureCuratedListIds, fetchCustomListMangaIds } from "./curated-lists";
import type { ChapterResponse, Metadata, SearchResponse } from "./models";
import { fetchJSON } from "./network";
import { assertDataArray, parseChapterTitle, parseMangaList } from "./parsers";
...
import { buildLatestChaptersUrl, buildMangaListUrl } from "./urls";
import {
  MANGA_PAGE_LIMIT,
  chunk,
  computeNextMetadata,
...
/** Matches the window the MangaDex website uses for "Popular New Titles". */
...
export async function getDiscoverSections(): Promise<DiscoverSection[]> {
  ...
  for (const sectionId of sectionOrder) {
    ...
    if (definition && definition.getEnabled()) {
      const type = SECTION_TYPE_MAP[definition.type];
      if (type) {
        sections.push({ id: definition.id, title: definition.title, type });
      }
    }
  }
  return sections;
